...
X = rGMM(Nsamp, d=2, k=2, M = means, S=list(Sigma, Sigma))
...
save_plot( "moderate_task.pdf",
  ggplot(X, aes(x=y1, y=y2)) + geom_point(color="royalblue") + geom_density2d(color="orangered") + lg_style
)
...
for(mr in miss_rate){
  ...
  X_miss[,1:2] = insertNA(as.matrix(X_miss[,1:2]), mr)
    ...
    save_plot(paste0("./moderate_task/MICE_", mr, ".pdf"),
    ggplot(D_imp, aes(x=imputed_value, y=real_value)) + geom_point() + geom_density2d()
  )

    save_plot(paste0("./moderate_task/MICE_filtered_", mr, ".pdf"),
    ...
    save_plot(paste0("./moderate_task/missForest_", mr, ".pdf"),
    ggplot(D_imp, aes(x=imputed_value, y=real_value)) + geom_point() + geom_density2d()
  )
    ...
    length(true_val)
    dist_table[as.character(mr), 'KNN'] = cor(imp_val, true_val, use='pairwise.complete.obs')

    save_plot(paste0("./moderate_task/KNN_", mr, ".pdf"),
    ggplot(D_imp, aes(x=imputed_value, y=real_value)) + geom_point() + geom_density2d()
  )
...
write.csv(dist_table, 'correlation_moderate_task.csv')
