...
compute_silh <- function(dat2, metric="euclidean")
{
    dat2['Silhouette'] = 0
    ...
    for(i in 1:dim(dat2)[1])
    {
        ...
        {
            Dc = cdist(dat2[i, coli], dat2[dat2$cluster==cl, coli], metric= metric )
   ...
   metric="euclidean",always.random=F)
{
    ...
    if(fix.means)
    {
      print("FIX initialize")
        ...
        if((sum(complete.cases(dat2)) < (dim(dat2)[2]*cn*5)) || (always.random))
        {
            ...
            print(L$M0)
            print(L$S0)
            Mt = fit.GMM(as.matrix(dat2), M0=L$M0, S0=L$S0,
             L$pi0, k=cn,report=TRUE)
        }
        else{
    ...
    dat2["entropy"] = apply(Mt@Responsibilities, 1, shannon_entropy)
    dat2 = compute_silh(dat2, metric = metric)
     ...
     prefix_file='./Cluster')
{
    ...
    data_clust_perf = foreach( i = nclust, .combine="rbind") %do% {
        ...
        cl_res = compute_data_cluster(dat2, i, fix.means=fix.means)
...
compute_perf_by_k_bootstrap <- function(dat2,
    maxclust=10, repetition=10, fix.means=F)
{

    n = dim(dat2)[1]
    m = dim(dat2)[2]
    nrep = ((maxclust-2)*repetition)
    ...
    data_clust_perf = foreach( i = 1:nrep, .combine="rbind") %do% {
        ...
        print(head(dat2))
        cl_res = compute_data_cluster(dat2[ids,], nc, fix.means=fix.means)
        Mt = cl_res$MNMmix
